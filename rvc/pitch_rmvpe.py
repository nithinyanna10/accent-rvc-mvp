...
class RMVPExtractor:
    ...
    def _load_model(self):
        ...
        for name in ("rmvpe.pt", "rmvpe.onnx"):
            ...
            if p.exists() and p.suffix == ".pt":
                loaded = torch.load(p, map_location=self.device, weights_only=False)
                # RVC rmvpe.pt is often a state_dict; we have no RMVPE model class here
                if isinstance(loaded, dict):  # RVC rmvpe.pt is state_dict only
                    self._model = "state_dict"  # marker: use fallback f0
                    return self._model
                self._model = loaded
    ...
    def extract_f0(
        ...
        # RVC rmvpe.pt is a state_dict; use librosa pyin as fallback when we have no model class
        if model == "state_dict":
            return self._extract_f0_librosa(wav, n_frames)
    ...
    def _extract_f0_librosa(self, wav: np.ndarray, n_frames: int) -> np.ndarray:
        """Fallback f0 using librosa.pyin when RMVPE checkpoint is state_dict only."""
        import librosa
